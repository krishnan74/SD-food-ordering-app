...
router.post(
  "/createFoodItem",
  [
    body("name", "Food name is required").not().isEmpty(),
    body("CategoryName", "Category name is required").not().isEmpty(),
    body("options", "Food options are required").isObject(),
    body("img", "Image URL is required").not().isEmpty().isURL(),
  ],
  async (req, res) => {
    let success = false;
    ...
    const { name, CategoryName, options, img } = req.body;
    ...
    try {
      ...
      success = true;
      res.json({ success, message: "Food item created successfully" });
    } catch (error) {
